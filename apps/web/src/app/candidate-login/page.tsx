...
function CandidateLoginPage() {
  ...
  useEffect(() => {
    ...
    if (candidateToken || ecosystemToken) {
  ...
  */
  ...
  async function verifyEmailOtp() {
    try {
      ...
      const res = await fetch('http://localhost:5000/api/v1/auth/candidate-login', {
        method: 'POST',
      ...
      localStorage.setItem('candidate_token', tokenData.accessToken);

      localStorage.setItem('candidate_refresh_token', tokenData.refreshToken);
  ...
  async function verifyMobileOtp() {
    try {
      ...
      localStorage.setItem('candidate_token', data.data.accessToken);

      localStorage.setItem('candidate_refresh_token', data.data.refreshToken);
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0f111a] px-4 py-8 font-sans text-white">
      <div className="w-full max-w-md rounded-2xl border border-[#2e354f] bg-[#181b28]/80 p-8 shadow-2xl backdrop-blur-md">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-400">Candidate Portal</p>
        </div>
